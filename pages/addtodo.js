  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (response && response.data) {
        ...
        if (response.status) {
          onAddTodo({
            id: items.length > 0 ? items.length + 1 : response.data.id, // this is because json place holder send the same id all the time you successfully create but we need the id to be unique
